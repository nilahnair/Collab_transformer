...
from torch.nn import TransformerEncoder, TransformerEncoderLayer #from raphael's code
...
class Network(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.config["input_embedding"]=="cnn":    
        ...
        elif self.config["input_embedding"]=="mlp":
            self.fc_input_embedding = nn.ModuleList()
    ...
    def embedding_def(self, x):
        if self.config["input_embedding"]=="cnn":
            for conv_layer in self.input_proj:
                x = conv_layer(x) 
            
        elif self.config["input_embedding"]=="mlp":
            fc_concat_embedding = create_mlp(input_dim=self.dim_merged_embedded, output_dim=0, net_arch=[self.dim_fully_connected],
                                             activation_fn= nn.ReLU, squash_output=False,)
        else:
            fc_concat_embedding = [nn.Linear(self.dim_merged_embedded, self.dim_fully_connected, bias=embedding_bias)]
        self.fc_concat_embedding = th.nn.Sequential(*fc_concat_embedding)
        #elif self.config["input_embedding"]=="time2vec":
